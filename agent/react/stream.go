...
func (a *ReActAgent) fireStreamEvent(ctx context.Context, ev hook.Event) error {
...
func (a *ReActAgent) runModel(
	...
	if err := a.fireStreamEvent(ctx, pre); err != nil {
		return nil, err
	}
	...
	if !useStream {
		...
		if err != nil {
			_ = a.fireStreamEvent(ctx, &hook.ErrorEvent{
		...
		_ = a.fireStreamEvent(ctx, &hook.PostReasoningEvent{
	...
	for chunk := range ch {
		...
		if chunk.Delta != "" {
			...
			if err := a.fireStreamEvent(ctx, &hook.ReasoningChunkEvent{
