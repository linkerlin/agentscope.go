...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/v2/chat/stream` | POST | SSE 事件流 |
| `/v2/chat/ws` | GET | WebSocket 双向流 |
| `/v2/resume` | POST | 恢复挂起的 Agent |
| `/api/v1/auth/register` | POST | 用户注册 |
| `/api/v1/auth/login` | POST | 用户登录 |
| `/api/v1/agents` | CRUD | Agent 管理 |
| `/api/v1/sessions` | CRUD | Session 管理 |
| `/api/v1/credentials` | CRUD | 凭证管理 |
...
| `a2a` | Agent-to-Agent 协议（AgentCard / Task / SSE / Registry） |
| `schedule` | Cron 调度器 |
| `async` | 异步任务执行池 |
